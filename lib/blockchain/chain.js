...
Chain.prototype.verify = async function verify(block, prev, flags) {
  ...
  //
  // This test fails for bitcoin edu for height <= 16
  // (due to encoding problems in Opcode.fromData)
  // Solution: don't enforce bip34 until maybe block 17
  if (state.hasBIP34()) {
...
Chain.prototype.getDeployments = async function getDeployments(time, prev) {
  ...
  // 
  // For Bitcoin Edu, CSV is on from beginning
  ...
  // 
  // For Bitcoin Edu, segwit is on from beginning
  // const witness = await this.getState(prev, deployments.segwit);
  const witness = thresholdStates.ACTIVE;
